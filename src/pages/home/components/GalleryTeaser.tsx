    ...
    <section
      ...
      <div
        ...
        <ul
          ...
          {HOME_GALLERY_TEASER.map(item => (
            <li key={item.id} className="group">
              <Link
                to={GALLERY_TEASER_VIEW_MORE.path}
                className="gallery-teaser-link relative block overflow-hidden rounded-lg bg-black shadow-sm ring-1 ring-black/5 focus-visible:outline-2 focus-visible:outline-offset-2 md:rounded-xl"
                aria-label={`${item.alt} — view full gallery`}>
                <img
                  src={item.imageSrc}
                  alt={item.alt}
                  width={item.width}
        ...
        <div className="scroll-reveal-item container-site mt-10 flex justify-center [transition-delay:160ms] md:mt-12">
          <Link
            to={GALLERY_TEASER_VIEW_MORE.path}
            className="site-btn site-btn-outline inline-flex min-h-11 items-center justify-center gap-2 px-8 py-3 text-sm font-semibold md:text-base">
